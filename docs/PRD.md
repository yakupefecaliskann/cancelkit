...
| C6 | ~~**Faturalama (bizim)**~~ | **KALDIRILDI (2026-08-01).** Lemon Squeezy checkout + 14 günlük trial + abonelik kapısı olarak inşa edilmişti; LS mağazası KYC onayı alamadığı ve hiçbir hesap ücretlendirilmediği için tüm faturalama katmanı sökülüp ürün ücretsizleştirildi. Yerine başka bir sağlayıcı konmadı. | — (gereksinim iptal) |
...
→ Dashboard'da kurtarılan MRR birikir (ürün ücretsiz — ödeme adımı yok)
```
...
| Aktif kurulum (widget canlı çalışan proje) | 20 |

> Gelir metrikleri (ödeme yapan müşteri, trial→paid dönüşüm, bizim churn'ümüz) 2026-08-01'de ürün ücretsizleştiği için kaldırıldı.

---

## 7. Gelir Modeli

**Şu an gelir modeli yok (2026-08-01).** Ürün ücretsiz: ödeme sağlayıcısı, ücretli plan ve deneme süresi yok. Lemon Squeezy tabanlı $29/ay planı, mağaza KYC onayı alamadığı ve hiçbir hesap ücretlendirilmediği için tamamen sökülmüştü. Ücretlendirme yeniden gündeme gelirse önce sağlayıcı kararı verilecek; mevcut kullanıcılar açık onay vermeden ücretlendirilmeyecek (bkz. `/terms` §3).
